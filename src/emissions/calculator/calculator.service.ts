...
export class CalculatorService {
   ...
   */
  async calculate(input: CalculationInput, orgId: string): Promise<CalculationResult> {
    return this.prisma.asOrg(orgId, async tx => {
      ...
      // ---- Storage tanks ----
      const tanks = facility.equipment.filter(e => e.category === 'STORAGE_TANK')
      ...
      for (const comp of compressors) {
        const factor = await this.lookupFactor(tx, 'COMPRESSOR_RECIPROCATING', 'CH4')
        if (!factor) continue
   ...
   */
  async persistResults(orgId: string, result: CalculationResult): Promise<number> {
    return this.prisma.asOrg(orgId, async tx => {
      const facility = await tx.facility.findFirst({
      ...
      for (const rec of result.records) {
        await tx.emissionRecord.create({
          data: {
            ...
            emissionSource: this.inferEmissionSource(rec.equipmentCategory) as any,
  ...
  private async lookupFactor(
    ...
    if (factors.length === 0) {
      this.logger.warn(`No emission factor found for ${equipmentCategory}/${pollutant}`)
      return null
    }
    ...
    if (equipmentCategory === 'PNEUMATIC_CONTROLLER' && factors.length > 1) {
      if (pneumaticType === 'high-bleed') {
        chosen =
          factors.find((f: any) => f.notes?.toLowerCase().includes('high-bleed')) ?? chosen
      } else {
        chosen =
          factors.find((f: any) => f.notes?.toLowerCase().includes('low-bleed')) ?? chosen
      }
    }
  ...
  private resolveTankThroughput(
    ...
    return (
      capacity * DEFAULT_TANK_TURNOVERS_PER_YEAR * fractionOfYear(periodStart, periodEnd)
    )
  }
