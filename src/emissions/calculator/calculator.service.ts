...
/** Default annual tank turnover when throughput isn't specified. */
const DEFAULT_TANK_TURNOVERS_PER_YEAR = 12;

/** Heuristic fugitive components per equipment item when not specified. */
...
export class CalculatorService {
  ...
  async calculate(
    ...
    for (const tank of tanks) {
      const factor = await this.lookupFactor("STORAGE_TANK", "VOC");
      if (!factor) continue;
    ...
    for (const comp of compressors) {
      const factor = await this.lookupFactor("COMPRESSOR_RECIPROCATING", "CH4");
      if (!factor) continue;
    ...
    const factor = await this.lookupFactor("FUGITIVE_COMPONENT", "CH4");
    if (factor) {
   ...
   */
  async persistResults(result: CalculationResult): Promise<number> {
    const facility = await this.prisma.facility.findUnique({
      where: { id: result.facilityId },
      select: { orgId: true },
    });
   ...
   * For pneumatic controllers, picks high-bleed vs low-bleed based on the pneumaticType.
   */
  private async lookupFactor(
    ...
    // For pneumatic controllers, multiple factors exist (high-bleed vs low-bleed).
    // Pick the one matching the controller type — defaulting to low-bleed when unspecified
    // (since OOOOb effectively bans new high-bleed installations).
    if (equipmentCategory === "PNEUMATIC_CONTROLLER" && factors.length > 1) {
      if (pneumaticType === "high-bleed") {
        chosen =
          factors.find((f) => f.notes?.toLowerCase().includes("high-bleed")) ??
          chosen;
      } else {
        chosen =
          factors.find((f) => f.notes?.toLowerCase().includes("low-bleed")) ??
          chosen;
      }
    }
  ...
  /**
   * Resolve the storage tank throughput. Prefer explicit override, else estimate
   * from capacity × default turnovers, prorated to the period.
   */
  ...
  /** Heuristic — most reciprocating compressors in the field have 2 cylinders. */
  ...
  /**
   * Sum records into top-level totals — CO2e plus a per-pollutant breakdown.
   */
  private aggregate(
    input: CalculationInput,
