...
export function EquipmentForm({
  ...
  const [category, setCategory] = useState<EquipmentCategory>('PNEUMATIC_CONTROLLER')
  ...
  // Category-specific
  const [pneumaticType, setPneumaticType] =
    useState<'high-bleed' | 'low-bleed' | 'instrument'>('low-bleed')
  ...
  useEffect(() => {
    ...
    if (equipment) {
      ...
      setInstallDate(equipment.installDate ? equipment.installDate.slice(0, 10) : '')
      setPneumaticType(
        (equipment.pneumaticType as 'high-bleed' | 'low-bleed' | 'instrument') ?? 'low-bleed',
      )
      setTankCapacityBbls(
        equipment.tankCapacityBbls != null ? String(equipment.tankCapacityBbls) : '',
      )
      setCompressorHp(equipment.compressorHp != null ? String(equipment.compressorHp) : '')
      setThroughputMcfd(
        equipment.throughputMcfd != null ? String(equipment.throughputMcfd) : '',
      )
    } else {
      setTag('')
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (category === 'PNEUMATIC_CONTROLLER' || category === 'PNEUMATIC_PUMP') {
      ...
      if (
        (category === 'COMPRESSOR_RECIPROCATING' || category === 'COMPRESSOR_CENTRIFUGAL') &&
        compressorHp
      ) {
  ...
  const isPneumatic = category === 'PNEUMATIC_CONTROLLER' || category === 'PNEUMATIC_PUMP'
  ...
  return (
    <Sheet
      ...
      subtitle={isEdit ? equipment?.tag : 'Tag a new asset for compliance tracking'}
      footer={
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        {(isPneumatic || isTank || isCompressor) && (
          <FieldGroup label="Emissions attributes">
            {isPneumatic && (
              ...
              >
                <Select
                  ...
                  onChange={e =>
                    setPneumaticType(
                      e.target.value as 'high-bleed' | 'low-bleed' | 'instrument',
                    )
                  }
                >
                  <option value="high-bleed">High-bleed (&gt;6 scf/hr)</option>
                  <option value="low-bleed">Low-bleed (≤6 scf/hr)</option>
            ...
            <Field label="Throughput (mcf/day)" hint="Optional — used for capacity checks">
              <Input
                type="number"
                step="any"
